...
> **구현 중 KTD3·KTD4가 두 번 고쳐졌다 (코드 리뷰).** 아래 원문은 남겨 두고 무엇이 왜
> 틀렸는지를 **KTD10·KTD11**에 적었다 — 둘 다 "테스트가 초록인 채로 기능만 없는" 종류라,
> 되돌리려는 다음 사람이 원문만 읽으면 같은 자리로 돌아간다.
...
**KTD10 — 법선은 최근접점이 아니라 닿는 순간의 상대 위치다** (KTD3 정정).
`dist2_to_segment`가 주는 최근접점을 그대로 법선으로 쓰면 **판정이 통째로 죽는다.**
최근접이 구간 **내부**라는 것은 이번 틱에 서로를 지나쳤다는 뜻이고, 그때 최근접 방향은
정의상 상대 변위와 직교한다 — 상대속도의 법선 성분이 0이 되어 KTD4의 접근 판정이
**스침을 전부 기각한다.** 즉 이 설계가 존재하는 이유인 경우만 정확히 빠진다. 증상은
"부딪혔는데 아무 일도 안 일어난다" 하나뿐이고, 정면(세로 오프셋 0)으로 시험하면 상대
위치가 원점을 지나 퇴화 경로로 빠지므로 **테스트가 초록으로 통과한다.**
고친 방법: `dist2_to_segment`는 **넓은 그물**로만 쓰고, 걸리면 `|R0 + t·D| = 반경`의
첫 근을 직접 풀어 그 시각의 상대 위치를 법선으로 쓴다.

**KTD11 — 상대속도는 `vx`/`vy`가 아니라 이번 틱에 지나온 거리로 잰다** (KTD4 정정).
`vx`/`vy`는 **던져졌을 때만** 0이 아니다(`Pet`의 필드 주석). 그래서 (1) 한 틱 안에
날아와 착지까지 끝낸 마리는 틱 끝에 속도가 0이고 — 넓은 화면·밀린 틱(250ms)에서 정확히
그렇게 된다 — (2) 미끄러지거나 헤엄치거나 발작하는 마리는 처음부터 0이다. 화면을
가로질러 놓고 판정에는 서 있는 것으로 보인다.
고친 방법: `Sweep`(틱 시작 중심 + 걸린 시간)을 `step_all`이 만들어 넘기고, 판정이 거기서
평균 속도를 뽑는다. 대신 걷기까지 전부 보이게 되므로 `PINBALL_BUMP_MIN_SPEED`(120)가
문턱이다 — 마주 걸어오는 두 마리는 84라 아래, 굴러떨어지기(200)·미끄러지기(220)·
발작(480)은 위. **R6의 근거가 바뀌었다**: 겹쳐 걷는 두 마리가 안 밀리는 이유는
"다가갈 때만"이 아니라 이 문턱과 그 조건이 **함께** 막기 때문이다.

**KTD12 — 볼링 판이 도는 동안에는 마리 간 판정을 쉰다.**
볼링 버튼과 핀볼 토글은 서로를 끄지 않아 동시에 켜질 수 있다. 그러면 같은 마리를 두
물리가 두고 다툰다: 부딪혀 `Thrown`이 된 핀은 `board.knock`을 거치지 않고 판에서 빠져
**볼링 연쇄가 조용히 끊기고**, 판이 튕긴 핀(세계 폭 ×1.5)은 여기서 던지기 상한(×1.4)으로
도로 깎인다. 게이트는 `Pets::step_all` 한 곳이다 — 판이 도는 동안 핀은 판이 몬다.
