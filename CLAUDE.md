...
- **macOS의 클릭 통과는 창 단위다 — "이 부분만 통과"가 없다.** 투명해도 히트 테스트는
  알파를 안 보므로 244×220짜리 펫 창은 여백까지 통째로 클릭을 먹는다. 부분 통과를
  흉내 내려면 **커서 위치를 따라 창 단위로 껐다 켜는 수밖에 없고**, 그러면 판정 주체가
  갈린다: 통과 중에는 웹뷰에 포인터 이벤트가 안 오므로 **웹뷰는 스스로 되돌릴 수 없다.**
  그래서 웹뷰가 요청하고 Rust 틱만 되돌린다(`pet_bridge/hit.rs`). 되돌리는 눈이 하나뿐
  이므로 **그 눈이 멀면 펭귄을 영영 못 누른다** — 통과는 매 틱 근거를 갱신하는 동안에만
  유지되게 짜고, 커서·배율을 못 읽으면 즉시 클릭을 먹는 쪽으로 떨어뜨린다.
  덧붙여 **CSS `pointer-events`와 창 통과는 다른 층이다**: 전자는 웹뷰가 반응하지
  않게 할 뿐 네이티브 창은 클릭을 그대로 먹는다. 둘의 경계가 어긋나면
  "웹뷰는 반응하는데 창은 통과시키는" 갈래가 생기므로 **같은 상자를 보게 한다.**
  (`opacity: 0`은 히트 테스트를 **안** 막는다 — `visiblePainted`가 보는 것은 `visibility`다.)
