...
pub struct PetState {
    ...
    /// 웹뷰가 "포인터가 펭귄 밖이니 창을 통과시켜 달라"고 남긴 요청.
    ///
    /// **요청일 뿐 상태가 아니다.** 창에 실제로 플래그를 거는 것도, 되돌리는
    /// 것도 틱 하나가 한다 — 웹뷰는 통과 중에 이벤트를 못 받으므로 스스로
    /// 되돌릴 수 없다. 기본값(없음)은 언제나 "클릭을 먹는다"이고, 모든 실패가
    /// 그리로 떨어진다.
    ///
    /// **`pets`와 동시에 잡지 않는다.** 락 둘을 겹쳐 쥐면 잡는 순서가 갈리는
    /// 순간 데드락이고, 증상은 "앱이 통째로 멈춘다" 하나뿐이다
