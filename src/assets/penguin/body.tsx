 ...
 *
 * **셋으로 나뉘어 나간다.** 겹치는 순서가 곧 그리는 순서인데, 훌라 차림이
 * 몸통과 가까운쪽 날개 **사이**에 끼기 때문이다 (`index.tsx`가 조립한다).
 */

import { BEAK, FOOT, HOLE, INK, SNOW } from "../palette";

/** 그림자와 얼음 구멍. **`.pg-all` 밖이다** — 착지 포즈가 몸을 통째로 누르는데
 * 바닥에 깔린 이 둘까지 같이 눌리면 안 된다. */
...
/** 가까운쪽 날개. **훌라 차림보다 뒤에 그린다** — 앞에 두면 날개를 저을 때
 * 어깨끈이 날개를 덮는다. */
