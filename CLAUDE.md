...
M1(메뉴바 셸 + 뽀모도로 + 알림) 완료·머지됨. 다음은 **M2 — Notion TODO 통합**이다.
"시작 전 결정" 중 토큰 보관 방식은 Keychain으로 확정됐고, **기존 Notion DB 스키마 확인**(PRD R3)이
아직 남은 선행 조건이다 — 상태 속성(완료/일부/미완/기타) 유무를 모르면 M2 스키마를 설계할 수 없다.
