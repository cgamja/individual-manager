...
**상태는 상대와의 거리로 고른다** (`t >= until[i]`일 때만 다시 고른다):

| 거리 | 확률 | 지속 |
|---|---|---|
| `> REACH + 44` | hunt 72% / circle 28% | 220~600ms |
| `> NEAR` | swing 50% / hunt 18% / circle 18% / guard 14% | swing 150~240ms, 나머지 180~500ms |
| 그 이하(붙음) | swing 42% / guard 22% / back 22% / circle 14% | swing 150~240ms, 나머지 160~460ms |
...
- `back`: `x -= ux·step·1.15`, `y -= uy·step·0.8`
...
상태를 다시 고르는 순간에만 바꾼다:
- 상대가 없거나 · 쓰러졌거나 · 자기 자신이거나 · **26% 확률**이면 다시 고른다.
...
- 안 막혔으면 피격 수 +1, 상대가 이미 `hurt`가 아니면 `hurt` 180ms.
