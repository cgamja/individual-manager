...
- **AE2** (즉시 반영) Given 펭귄 두 마리가 걷고 있다. When 크기를 60%로 민다. Then 두
  마리 모두 그 자리에서 84px로 줄고, 걷던 동작이 이어지며, **발이 바닥선에 붙어 있다**
  (창이 다시 뜨거나 펭귄이 순간이동하지 않는다).
...
|---|---|---|---|---|
| 1 | **커서 방망이** `props/bat` | 펭귄 창 + 핀볼 판 창 | CSS `cursor: url(...) hx hy`. **`set_zoom`이 이걸 어떻게 다루는지가 갈림길**이라 P1 프로브로 정한다 (KTD9) | **별도 조치** |
...
| 3b | **코트** `props/court` | 코트 창(`volley-court.html`) | `court.css`가 `--vb-net-h: 85px` · `--vb-net-w: 96px` · `--vb-sand-depth: 80px`를 **고정 px**로 박고 `volley.test.ts`가 Rust 상수와 대조한다. 퍼센트가 아니라 자동이 아니다 → 창에 **`set_zoom(s)`** 를 건다 | **별도 조치** |
...
**KTD4 — 웹뷰의 확대는 CSS가 아니라 `WebviewWindow::set_zoom`으로 건다. 대상은 두 창이다.**

펭귄 창의 레이아웃은 `--pg-size`·`--pg-pad-x`·`--pg-pad-top`뿐 아니라 말풍선·낚시 구멍·
후광 두께 등 **수십 개의 px 값**이고, 코트 창도 `--vb-net-h: 85px`처럼 고정 px가 Rust
상수와 대조되고 있다. 이걸 전부 `calc(… * var(--s))`로 바꾸는 것은 diff도 크고
`pet-css.test.ts`·`volley.test.ts`의 대조를 깨뜨린다.

대신 Rust가 창을 만든 뒤 `window.set_zoom(s)`를 부른다. tauri 2.11.5에 있고
(`WebviewWindow::set_zoom`), macOS 구현은 wry 0.55.1의 `WKWebView.setPageZoom`이다
(**macOS 11+**, 이 앱의 대상은 macOS 26이라 무관).

**거는 창과 안 거는 창의 기준은 "고정 px가 있는가"다.**

| 창 | `set_zoom` | 근거 |
|---|---|---|
| 펭귄 (`pet-*`) | **건다** | CSS가 px로 짜여 있다 |
| 코트 (`volley-court`) | **건다** | `--vb-net-h` 등 고정 px |
| 볼링 공 (`bowling-ball`) | 안 건다 | 전부 `100%` — 창 크기만 바꾸면 따라온다. 줌을 걸어도 `100%`는 여전히 창 전체라 **아무 일도 안 일어난다** |
| 비치볼 (`volley-ball`) | 안 건다 | 같은 이유 |
| 핀볼 판 (`pinball-board-*`) | **안 건다 (확정)** | 그림이 없어 이 표의 기준("고정 px가 있는가")으로 대상이 아니다. **커서 하나 때문에 걸면 그 기준을 어기는 것**이다. 게다가 이 창은 `set_ignore_cursor_events`와 `ns_window()` 레벨 2가 동시에 걸린 창이고 `CLAUDE.md`의 함정 셋이 전부 이 창 위에서 나왔다 — **아는 것이 없는 축을 거기에 더 얹지 않는다** (2026-09-03 확정) |

`getBoundingClientRect()`와 `clientX`는 같은 줌 좌표계라 `PetApp`의 정규화 계산
(빠따 `nx/ny`, 시선)은 **줌과 무관하게 그대로 맞다.** 드래그는 `screenX/screenY`(줌 밖)를
쓰므로 Rust 커맨드에서 `to_core`로 나눈다 (KTD6).

**폴백** — `setPageZoom`이 투명·무장식 창에서 안 먹거나 그림이 깨지면: 펭귄 창은
`pet://scale` 브로드캐스트(`pet://sound`와 같은 패턴) + `loadPetSettings()`로 받아
`document.documentElement.style.zoom`을 걸고, 코트 창도 같은 경로를 쓴다. 정지 조건 1.
...
**KTD9 — 커서 방망이는 이 작업에서 가장 까다로운 조각이다. P1 프로브가 두 갈래를 가른다.**

문제가 셋이다.

1. **CSS `cursor`는 확대의 예외일 수 있다.** `cursor: url(...) hx hy`의 이미지 크기와
   핫스팟이 페이지 줌을 따라가는지가 WebKit 구현 세부이고, **추측으로 정할 수 없다.**
2. **핫스팟도 함께 스케일돼야 한다.** 지금 `bat.ts`는 `width='48' height='48'`
   `viewBox='0 0 48 48'`에 핫스팟 `"10 30"`이다. 이미지만 키우고 핫스팟을 두면 **클릭한
   곳과 방망이 끝이 어긋난다.**
3. **두 창에 있다.** 펭귄 창(`css/pinball.css`)과 핀볼 판 창(`pinball.css`)이 같은
   프로퍼티(`--pg-bat-cursor`/`--pg-bat-swing`)를 쓰는데, **두 창의 줌 상태가 다를 수 있다.**

**P1 프로브** — **U3가 끝난 뒤** 실측한다. 줌이 존재해야 관측이 되므로 U1 앞에 둘 수 없다.
핀볼 모드를 켜면 **한 화면에 A/B가 동시에 뜬다**: 펭귄 창(줌 `s`)과 핀볼 판(줌 1)이 같은
`--pg-bat-cursor` 값을 쓰므로, 크기를 150%로 두고 커서를 펭귄 위 → 판 위로 옮기며
`screencapture -C`(커서를 함께 찍는다)로 두 장을 비교하면 된다.

**이미지 크기와 핫스팟을 함께 본다.** 이미지만 커지고 핫스팟이 그대로면 그것은 결과 A가
**아니다** — 방망이 끝이 포인터에서 어긋난 채로는 때리는 위치가 틀어진다.
**애매하면 B로 떨어진다.**

- **결과 A — 줌이 이미지와 핫스팟을 함께 키운다**
  → **펭귄 창은 줌으로 공짜**(코드 변경 없음), **핀볼 판 창은 `installBatCursor` 명시 경로**.
  판 창은 줌이 1이라 이중 적용이 일어나지 않는다. **판 창에는 `set_zoom`을 걸지 않는다**
  (KTD4 확정).
- **결과 B — 줌이 커서를 안 키운다(또는 핫스팟이 안 따라온다)**
  → **두 창 다 `installBatCursor` 명시 경로.**

**명시 경로(두 결과가 공유한다)** — `installBatCursor(fallback, doc, scale)`에 배율 인자를
더한다. SVG는 `width=48×s height=48×s`, `viewBox`는 `0 0 48 48` 그대로(그림이 늘어난다),
핫스팟은 `10×s 30×s`. 배율은 **`pet://scale` 브로드캐스트**로 받는다 — `pet://sound`와
완전히 같은 패턴이고(`emit` → 각 창이 `listen`), `pet-*`·`pinball-board-*`가 이미
capabilities에 있어 권한 변경이 없다. 각 엔트리는 마운트 시 `loadPetSettings()`로 한 번
읽고, 이후 브로드캐스트마다 다시 심는다.

**결과 A에서 두 창의 경로가 갈리는 것은 섞인 게 아니다** — 그 창의 **줌 값이 다르기**
때문이다. 그 이유를 각 엔트리에 주석 한 줄로 남긴다(WHAT만; 판단 이력은 이 플랜에).

**어느 갈래든 지키는 것**

- **`cursor`의 키워드는 목록 맨 끝에만 온다.** `batCursorValue()`가 이미 값 전체
  (`url(…) 10 30, grab`)를 프로퍼티에 담고 CSS는 `cursor: var(--pg-bat-cursor, grab)`으로
  받는다 — **이 구조를 건드리지 않는다.** `cursor: var(--x, grab), grab`으로 쓰면 프로퍼티가
  없을 때 `grab, grab`이 되어 **선언이 통째로 버려진다**(CLAUDE.md 함정, `pinball-css.test.ts`가
  이미 대조한다).
- **커서 이미지 크기 상한.** WebKit은 사용자 지정 커서가 일정 크기(대략 **128×128 px**)를
  넘으면 **그림을 버리고 대체 키워드로 떨어진다** — "커서가 갑자기 화살표로 돌아간다"는
  증상 하나만 남는다. 지금 48px에 상한 150%면 **72px**이라 여유가 있지만, 그 여유를
  **테스트로 못 박는다**(`커서_이미지가_128px를_넘지_않는다`). 상한을 올리려면 이 테스트가
  먼저 빨개진다.
