...
pub struct Court {
    ...
    /// 세운 것과 같은 자리이고, **모래도 네트도 여기에 함께 뜬다.**
    play_y: f64,
...
impl Court {
    ...
    /// 네트 꼭대기 — 펭귄 머리 바로 밑이다.
    ///
    /// **모래에서 재 올린다.** `play_y + NET_DROP`과 값은 같지만(높이가
    /// `PET_SIZE - NET_DROP`이므로) 이렇게 써야 "그물은 모래에 발을 딛고
    /// 이만큼 솟는다"가 식에 그대로 남는다 — 판 좌상단에서 재 내리면 그물이
    /// 어디에 서 있는지가 식에서 사라진다.
...
impl Volleyball {
    ...
    /// **모래 위에 떨어지는 첫 등급**을 쓴다.
    ///
    /// **지금은 언제나 첫 등급이다.** 모래가 판의 발밑으로 돌아오면서 낙하 높이가
    /// 세계와 무관한 상수가 됐기 때문이다 — 판이 화면 세로 중앙이고 모래만 화면
    /// 바닥이던 시절에는 낙하가 화면 절반이라 세로로 긴 모니터에서 긴 등급이
    /// 실제로 필요했다. 훑기를 남겨 두는 것은 `REACH`·`SAND_DEPTH`·`NET_DROP`을
    /// 만졌을 때 그 전제가 조용히 깨지지 않게 하는 보험이고,
    /// `킬샷은_지금_언제나_가장_짧은_등급이다`가 현재 사실을 못 박는다.
    ...
    /// 테스트가 `kill_shot`만 떼어 보는 창구. 등급 훑기가 지금 어느 등급에서
    /// 끝나는지를 고정하는 데 쓴다.
    #[cfg(test)]
    pub(super) fn kill_shot_for_test(&self, x0: f64, 빈자리: f64, to: Side) -> (f64, u64) {
        self.kill_shot(x0, 빈자리, to)
    }
