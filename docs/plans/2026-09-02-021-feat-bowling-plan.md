...
- **실행 프로필** — **PR 둘로 나눈다** (2026-09-02 사용자 지시).
...
- **Patterns to follow** — `pet.rs` 4616줄 분할 PR(#43)의 합격 기준 방식: **테스트 이름
  다중집합 불변**. 여기서는 더 강하게, **테스트 파일을 한 줄도 고치지 않고** 전부 통과해야 한다.
- **Test scenarios**
  - `여러_마리를_한_번에_돌려도_따로_돌린_것과_같다` — 같은 시드의 `Pets` 둘을 만들어 한쪽은
    `step_all`로, 다른 쪽은 기존처럼 마리별 `step`으로 60초 굴린 뒤 최종 상태가 일치
...
- **Verification** — `cd src-tauri && cargo test` 전체 통과, **테스트 파일 diff 0줄**.
...
- **U1이 테스트 파일을 한 줄도 고치지 않고** 통과 — 동작 불변이 PR 1의 합격 기준이다
