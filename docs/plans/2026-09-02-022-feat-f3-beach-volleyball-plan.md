...
- **실행 프로필** — **PR 하나.** 브랜치 `feat/f3-beach-volleyball-01`.
  - **구현은 직렬이다** (2026-09-02 사용자 지시). 플랜을 먼저 쓰고 구현을 뒤에 붙였다.
    **스윙 넉백(#51)은 머지됐고** 그 위로 리베이스했다 (`cargo test` 기준선 291 → 310).
    **핀볼 마리 간 충돌은 아직 돌고 있다** — 그쪽이 `Pets::step_all` **본문**,
    `pet/motion/pinball.rs`, `tuning.rs`의 "── 핀볼 모드 ──" 절, `pet_bridge/tick.rs`를
    갖는다. 내 삽입점은 §"공유 파일 삽입점"에 있고, 그 PR이 먼저 머지되면 다시 리베이스한다 —
    **양쪽을 다 살린다.**
...
- **정지 조건** — 아래 중 하나라도 걸리면 멈추고 보고한다.
  - **코트 창이 실제로 클릭을 통과시키지 못한다** — 스파이크로 먹는 것은 확인했으나
    (KTD5), 실제 코트 창에서 다르게 나오면 "그림뿐인 창이 화면을 먹는" 상태가 되고
    PRINCIPLE 5를 근거 없이 어긴다.
...
**성립한다 — 실측으로 확인했다** (2026-09-02, 사용자 스파이크). 화면 전체를 덮는
투명 + `always_on_top` 창에서 `set_ignore_cursor_events(true)` 뒤 `ignoresMouseEvents=true`,
`level=5`가 그대로 유지됐다. `always_on_top`을 다시 걸어도 안 풀린다. macOS에서 클릭 통과는
`NSWindow.ignoresMouseEvents`가 곧 메커니즘이므로 이걸로 답이 됐다 (Q5는 닫혔다).

**대신 함정이 딸려 나왔다 — 이 세터는 비동기다.** `Ok(())`를 즉시 돌려주지만 적용은 이벤트
루프를 거친다(`set_position`이 메인 스레드로 디스패치되는 것과 같은 성질). 그래서:

1. **적용됐는지 읽어서 확인하는 코드를 호출 직후에 두지 않는다.** 첫 스파이크가 정확히 그렇게
   해서 "안 먹는다"는 오답을 냈다 — 반환값도 `Ok`, 로그도 깨끗한데 읽으면 `false`인,
   이 레포가 문서화해 온 조용한 실패와 같은 모양이다.
2. **창을 `visible(false)`로 만들고, 플래그를 건 뒤, `show()` 한다.** 둘 다 같은 이벤트 루프를
   순서대로 지나므로 이게 간극을 가장 좁힌다. 그래도 세터가 비동기라 **한 프레임쯤은 남을 수
   있다.** 그 한 프레임을 감수하는 근거: 코트가 뜨는 순간 포인터는 방금 버튼을 누른 **설정 창
   위**에 있지 코트 위가 아니고, 코트는 20초 뒤 사라진다. 이 한 프레임을 없애자고 창 생성을
   지연시키면 "버튼을 눌렀는데 아무 일도 안 일어나는" 구간이 대신 생긴다.
3. 이 발견은 `docs/solutions/`에 남긴다 (U10).

창 레벨은 별개다: 통과시키는 것(`ignore_cursor_events`)과 뒤에 두는 것(`setLevel`)은 서로를
대신하지 못한다 — 레벨만 내리면 판을 한 번 클릭했을 때 도로 올라온다
...
### U10. `set_ignore_cursor_events`의 비동기 함정을 기록한다

- **Goal** — 다음에 같은 증상("먹는 줄 알았는데 읽으면 `false`")을 만난 사람의 탐색 공간을 줄인다.
- **Requirements** — KTD5
- **Dependencies** — U5 (실제 코트 창에서 확인한 뒤에 쓴다)
- **Files**
  - `docs/solutions/best-practices/tauri-ignore-cursor-events-is-async.md` (신규)
  - `CLAUDE.md` — 함정 목록에 한 줄
- **Approach** — 기존 문서 형식(증상 / 안 통한 시도 / 해결 / 왜 통하는지 / 예방책)을 따른다.
  **"안 통한 시도"에 "호출 직후 읽어서 `false`를 보고 안 먹는 줄 알았다"를 반드시 넣는다** —
  이 항목이 이 문서의 존재 이유다. 실측값(즉시 `false` → 2초 뒤 `true`, 레벨은 안 변함)을
  그대로 적는다.
- **Test scenarios** — `Test expectation: none — 문서.`
- **Verification** — `CLAUDE.md` 함정 목록의 다른 줄들과 같은 꼴인지 눈으로 확인.
...
### 사용자가 판단해야 하는 것 셋 (열린 질문, 구현으로는 못 닫는다)

스모크에서 **눈으로 보고 답해야** 하는 것들이다. 셋 다 "무엇을 바꾸면 되는지"까지 적어 둔다 —
답이 "별로"여도 다음 수가 상수 하나이므로 되돌리기가 싸다.

- **Q1 — 두 마리(1대1)가 지루한가?** (스모크 1~6번에서 본다)
  KTD3의 갈래 다섯 중 (1)"누가 받으러 뛰는가"가 1대1에서는 죽는다 — 언제나 그 한 마리다.
  베팅은 *"뛰는 거리가 코트 반쪽을 다 쓰므로 오히려 더 많이 뛴다"*이고, **눈으로만 확인된다.**
  → 지루하면: `VOLLEY_COURT_HALF`를 줄여 왕복을 빠르게 하거나, 다이빙 리시브(Deferred)를 당긴다.
- **Q2 — 20초가 맞는 길이인가?** (스모크 4번에서 시간을 잰다)
  얼음낚시(30~60초, 배경)와 볼링(5~10초, 내가 한 짓) 사이의 빈 자리라 비교 대상이 없다.
  → 길면 `VOLLEY_SESSION_MS`를 `(13_000, 16_000)`쯤으로 내린다. 상수 **하나**다.
- **Q3 — 끝이 항상 스파이크인 것이 예측 가능해서 시시한가?** (스모크 5번, 19번에서 세 판을 비교)
  PRINCIPLE 1의 *"재미의 반대말은 예측 가능함"*과 정면으로 부딪히는 유일한 자리다. 예산 종료
  (KTD7)의 **종료 증명**과 맞바꾼 것이라 공짜로는 못 없앤다.
  → 시시하면 후보 둘. (a) `VOLLEY_FLIGHT_MS[0]`(킬샷 체공)에 갈래를 줘서 마지막 공만 스파이크와
  기습 페인트로 나눈다 — 종료 증명이 유지된다. (b) 랠리 중에도 낮은 확률로 헛치기를 넣는다 —
  **판이 4초 만에 끝나는 경우가 생겨 Q2의 "20초쯤"을 깬다.** (a)를 먼저 본다.
...
- **Q4 — 코트 창의 등장·퇴장 애니메이션과 창 생성 시점이 어긋나지 않는가.** 창이 생기는
  ...
  확인 항목이다 (스모크 2번).
- **~~Q5 — `set_ignore_cursor_events`가 `always_on_top` 창에서도 먹는가.~~ 닫혔다**
  (2026-09-02 실측). 먹는다. 딸려 나온 비동기 함정은 KTD5에 적었고 `docs/solutions/`에 남긴다.
