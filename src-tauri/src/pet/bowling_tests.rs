...
fn 맞은_핀은_튕겨_나간다() {
    ...
    // **한 틱 뒤 위치로 보면 안 된다.** 튕기는 속도가 세계 폭에 비례하므로 핀이
    // 50ms 안에 벽까지 가서 되튀어 돌아온다 — 그 순간을 찍으면 "왼쪽으로 갔다"로
    // 읽힌다. 이 판정은 옛 속도에서 **0.25px 차이로** 통과하고 있었고, 던지기
    // 상한을 올리자 부호가 뒤집혔다. 밀린 **방향**을 보려면 날아가는 동안의
    // 가장 먼 지점을 봐야 한다.
    let 가장_멀리 = (1..=30)
        .map(|i| {
            pets.step_all(tt + 10 * i, |_| Some(&w));
