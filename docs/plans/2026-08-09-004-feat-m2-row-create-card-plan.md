---
title: M2 행 만들기 확장 (특수 행·미래 [TODO]·아이콘 복사) - Plan
type: feat
date: 2026-08-09
...
# M2 행 만들기 확장 (특수 행·미래 [TODO]·아이콘 복사) - Plan

## Goal Capsule

- **목표**: TodoCard에 인라인 "행 만들기" 폼을 추가해 계획표 DB에 특수 행(휴일·MT류)과 미래 날짜의 [TODO] 행을 앱 안에서 만들고, 생성(또는 기존 행 열기) 직후 카드가 그 날짜 페이지로 전환되어 할 일을 바로 편집한다. 함께 기존 '오늘 페이지 만들기'의 아이콘 누락을 최신 [TODO] 행 아이콘 복사로 고친다. TODO.md에 없는 신규 항목이므로 PRD.md·SERVICES.md·TODO.md 개정을 포함한다.
...
TodoCard가 카드 안에서 펼쳐지는 "행 만들기" 폼을 얻는다 — 제목 프리셋 칩([TODO]·휴일·MT·직접 입력) 하나로 특수 행과 미래 [TODO] 행을 같은 폼에서 만들고, 만든(또는 이미 있던) 날짜의 페이지로 카드가 전환되어 할 일을 바로 입력한다. 기존 '오늘 페이지 만들기'와 폼의 [TODO] 생성은 최신 [TODO] 행의 아이콘을 복사해 붙인다.
...
- **통합 진입점.** 특수 행과 미래 [TODO] 행을 별도 기능으로 나누지 않고 한 폼에서 제목 프리셋으로 갈라진다. (session-settled: user-directed — 특수 행 전용 폼 + 기존 버튼 확장의 이원화 대비: 진입점 하나로 통합.)
...
- **날짜 범위는 특수 행 전용이고 행 하나에 시작~끝을 담는다.** 실제 DB의 범위 3건이 모두 특수 행 유형이다. [TODO] 행은 하루 단위만 만든다.
...
- **수행도는 특수 행에만 쓴다.** 특수 행은 기본 '기타'로 지정해 생성하고, [TODO] 행은 기존 '오늘 페이지 만들기'와 동일하게 비워 둔다. (session-settled: user-approved — [TODO]에도 기본값을 쓰는 안 대비: 하루가 끝나야 정해지는 값이라 비워 두는 쪽을 확인받음.)
...
- R2. 제목은 프리셋 칩([TODO]·휴일·MT)과 직접 입력을 제공하며, `[TODO]`가 아닌 제목은 특수 행으로 취급한다.
- R3. 특수 행은 아이콘(이모지 1자)·날짜(하루 또는 시작~끝 범위, 행 하나)·수행도(기본 '기타')를 지정해 본문 골격 없이 생성한다.
- R4. [TODO] 행은 날짜 하루만 받아 골격(`공부`/`기타` 헤딩) 포함으로 생성하고, 아이콘은 복사 규칙(R9)을 따르며 수행도는 비워 둔다 — 수동 아이콘·수행도 필드를 노출하지 않는다.
- R5. 선택한 날짜에 같은 성격의 행이 이미 있으면 — [TODO] 생성 시 그 날짜의 [TODO] 행, 특수 행 생성 시 같은 제목의 행 — 생성하지 않고 알림과 함께 "기존 행 열기"를 제공한다. 성격이 다른 행의 공존(예: [TODO]가 있는 날의 휴일 행 추가)은 정상 생성이다.

**생성·열기 후 날짜 전환**

- R6. 생성 성공 또는 "기존 행 열기" 시 카드가 그 날짜 페이지의 스냅샷으로 전환되고, 조회·추가·토글·편집이 오늘 페이지와 동일하게 동작한다(특수 행 포함).
...
```mermaid
flowchart TB
  A["행 만들기 버튼"] --> B["인라인 폼<br/>제목 칩 · 날짜 · (특수: 아이콘·수행도)"]
  B --> C{"같은 성격의 행이<br/>그 날짜에 있나?"}
  C -->|없음| D["행 생성<br/>특수: 골격 없음 / TODO: 골격+아이콘 복사"]
  C -->|있음| E["기존 행 열기"]
  D --> F["카드가 그 날짜로 전환<br/>할 일 조회·추가·편집"]
  E --> F
  F --> G["오늘로 돌아가기<br/>(팝오버 재열림 시 자동)"]
```

- F1. 특수 행 생성
  - **Trigger:** 폼에서 휴일 칩 선택, 아이콘·날짜(범위 가능)·수행도 지정 후 만들기.
  - **Steps:** 중복 검사(같은 제목) → 행 생성(골격 없음) → 카드가 그 날짜로 전환.
  - **Covers:** R1–R3, R5, R6.
- F2. 미래 [TODO] 준비
  - **Trigger:** 폼에서 [TODO] 칩 + 미래 하루 선택 후 만들기.
  - **Steps:** 중복 검사([TODO] 행) → 골격+복사 아이콘으로 생성 → 카드 전환 → 할 일 입력 → 오늘로 복귀.
  - **Covers:** R1, R2, R4–R7, R9.
- F3. 중복 날짜
...
- AE1. **Given** 폼에 휴일·🏖️·8/12~8/14·수행도 기타 **When** 만들기 **Then** `날짜` 시작 8/12·끝 8/14인 행 하나가 본문 골격 없이 생성되고 카드가 그 페이지로 전환된다. (Covers R3, R6)
- AE2. **Given** [TODO] 칩 + 행 없는 미래 날짜 **When** 만들기 **Then** 골격과 최신 [TODO] 아이콘이 복사된 행이 생성되고, 전환된 카드에서 할 일을 추가할 수 있다. (Covers R4, R6, R9)
- AE3. **Given** 선택한 날짜에 이미 [TODO] 행 **When** [TODO] 만들기 **Then** 생성 없이 "기존 행 열기"가 제시되고, 열면 그 페이지로 전환된다. (Covers R5, R6)
- AE4. **Given** 그 날짜에 휴일 행만 있고 [TODO] 없음 **When** [TODO] 만들기 **Then** 정상 생성된다 — 공존 허용. (Covers R5)
