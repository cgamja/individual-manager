...
### 주석
- **WHAT을 간단히 적는다.** 이 코드가 무엇인지, 어떤 규약을 지켜야 하는지까지다
- **결정 과정·실패 이력을 주석에 쌓지 않는다.** "왜 그렇게 정했나"는 `PRD.md`·`MOTIONS.md`·
  `TODO.md`·`docs/plans/*`·`docs/solutions/*`에 있고, 필요하면 그쪽을 **한 줄로 가리킨다**
- 다시 밟으면 코드가 조용히 망가지는 함정은 **한두 줄로** 남긴다 (ex. "`var()` 뒤에
  키워드를 덧붙이지 않는다 — 없을 때 선언이 통째로 죽는다")
- 2026-09-02 020 리팩터링, 2026-09-03 에셋 리팩터링에서 두 번 같은 지시를 받았다
